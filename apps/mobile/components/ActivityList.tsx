...
import { profilesApi, type ActivityItem } from "../lib/api";
...
function NotifRow({ item }: { item: ActivityItem }) {
  ...
  return (
    ...
    >
      ...
      {item.type === "follow" ? (
        <Pressable
          className={following ? "bg-surface border border-border px-3 py-1 rounded-lg" : "bg-primary px-3 py-1 rounded-lg"}
          onPress={(e) => { e.stopPropagation?.(); handleFollow(); }}
        >
          <Text className={following ? "text-text font-semibold text-[12px]" : "text-white font-semibold text-[12px]"}>
            {following ? "Abonné" : "Suivre"}
          </Text>
        </Pressable>
      ) : item.postImage ? (
        <Image source={{ uri: item.postImage }} className="w-10 h-10 rounded-md" />
      ) : null}
    </Pressable>
  );
}
...
export function ActivityList({ ListHeaderComponent }: Props) {
  ...
  return (
    <FlatList
      ...
      ListEmptyComponent={() =>
        !isLoading ? (
          <View className="py-[60px] items-center gap-2">
            <Ionicons name="heart-outline" size={40} color={colors.textMuted} />
            <Text className="text-text-muted text-[14px]">Aucune activité</Text>
          </View>
        ) : null
      }
    />
